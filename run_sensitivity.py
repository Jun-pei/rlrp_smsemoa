...
run_sensitivity.py
==================
Sensitivity sweeps over the planner's hyper-parameters (Sec. 5.3), on the
Definition-D1/D4 indicators.  Only RL-RP-SMS-EMOA is run: the baselines do not
depend on W, rho, sigma0 or the Q-learning parameters.

    python3 run_sensitivity.py --problem imop8 --param rho --values 0.3 0.5 0.7 0.9
"""
...
import numpy as np
import pandas as pd
from rlrp_smsemoa.experiment import run_single, get_frame
...
def main():
    ...
    for v in a.values:
        ...
        for seed in range(a.n_seeds):
            r = run_single(a.problem, "RL-RP-SMS-EMOA", seed, a.t_max, m=m,
                           mu=mu, frame=frame, eval_every=a.eval_every, **kw)
            r[a.param] = val
            rows.append(r)
    ...
    cols = ["hvr_final", "igd_plus_final", "hvr_anytime", "zref_mean_final",
            "qcov_frac_visited"]
